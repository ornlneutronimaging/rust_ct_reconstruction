#!/usr/bin/env bash
# Launch the CT Reconstruction GUI, rebuilding first if the sources changed.
#
# Usage: ./launch_ct_reconstruction.sh
set -euo pipefail

REPO_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BINARY="$REPO_DIR/target/release/ct_reconstruction"
...
if $needs_build; then
    ...
    echo "Building ct_reconstruction (release)..."
